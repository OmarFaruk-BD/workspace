...
class _ConfettiAnimationState extends State<ConfettiAnimation>
  ...
  void startAnimation() {
    ...
    _velocitiesX = List.generate(
      100,
      (index) => (random.nextDouble() - 0.5) * 8,
    );
    _velocitiesY = List.generate(100, (index) => -random.nextDouble() * 16);

    _shapes = List.generate(
      100,
      (index) => ShapeType.values[random.nextInt(ShapeType.values.length)],
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          const Center(child: Text('Tap the button to see the magic!')),
          CustomPaint(
    ...
    for (int i = 0; i < positions.length; i++) {
      ...
      switch (shapes[i]) {
        ...
        case ShapeType.rectangle:
          canvas.drawRect(
            Rect.fromCenter(center: positions[i], width: 10, height: 10),
            paint,
          );
          break;
